...
ForEach ($technique in $techniques) {
    ...
    $csfolder = "$techniques_folder$technique\$technique.sln"
    if(Test-Path($binfolder)) {
        ...
        if(Test-Path($csfolder)) {
            ...
            $binlocalname = "$techniques_folder$technique\bin\Debug\$technique.exe"
            C:\Windows\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe $csfolder
            
            if(test-path($binlocalname)) {
                Copy-Item -Path $binlocalname -Destination $binname
            }
        } else {
